...
class VistaLogIn(Resource):
    def post(self):
        ...
        usuario = Usuario.query.filter_by(nombre = u_nombre, contrasenia = u_contrasenia).first()
        if usuario:
...
class VistaAlbum(Resource):
    ...
    def get(self, id_album):
        album = Album.query.get_or_404(id_album)
        usuario = Usuario.query.get_or_404(album.usuario)
        usuario_actual = get_jwt_identity()
        if usuario.nombre != usuario_actual:
            return 'No tienes permiso para acceder a este álbum', 403
